...
void BUTTON_init(void) {
  ...
  LPC_SC->EXTMODE = 0x7;             //XXX Fronte di discesa

  NVIC_EnableIRQ(EINT2_IRQn);              /* enable irq in nvic                 */
	//XXX Esempio cambio priorit¨¤
  //NVIC_SetPriority(EINT2_IRQn, 1); //XXX settiamo la priorit¨¤ massima per il tasto1
  NVIC_EnableIRQ(EINT1_IRQn);              /* enable irq in nvic                 */
  NVIC_EnableIRQ(EINT0_IRQn);              /* enable irq in nvic                 */
	//NVIC_SetPriority(EINT0_IRQn, 3); //XXX settiamo la priorit¨¤ minima per il tasto0
}
